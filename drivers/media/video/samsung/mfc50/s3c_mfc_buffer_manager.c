...
static s3c_mfc_alloc_mem_t *s3c_mfc_alloc_mem_head[MFC_MAX_PORT_NUM];
static s3c_mfc_alloc_mem_t *s3c_mfc_alloc_mem_tail[MFC_MAX_PORT_NUM];
static s3c_mfc_free_mem_t *s3c_mfc_free_mem_head[MFC_MAX_PORT_NUM];
static s3c_mfc_free_mem_t *s3c_mfc_free_mem_tail[MFC_MAX_PORT_NUM];
...
int s3c_mfc_init_buffer_manager(void)
{
	...
	for (port_no = 0; port_no < MFC_MAX_PORT_NUM; port_no++) {
		...
		/*
		 * init alloc list,
		 * if (s3c_mfc_alloc_mem_head == s3c_mfc_alloc_mem_tail) then,
		 * the list is NULL
		 */
		alloc_node =
		...
		/*
		 * init free list,
		 * if (s3c_mfc_free_mem_head == s3c_mfc_free_mem_tail) then,
		 * the list is NULL
		 */
		free_node =
...
s3c_mfc_alloc_mem_t *s3c_mfc_get_alloc_mem_head(int port_num)
{
	s3c_mfc_alloc_mem_t *alloc_mem_head;
...
s3c_mfc_alloc_mem_t *s3c_mfc_get_alloc_mem_tail(int port_num)
{
	s3c_mfc_alloc_mem_t *alloc_mem_tail;
