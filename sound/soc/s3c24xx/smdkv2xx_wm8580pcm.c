...
static void wait_stable(unsigned int mtime_out)
{
	if (!mtime_out)
		mtime_out = 1000;

	mtime_out = loops_per_jiffy / HZ * mtime_out;
...
static int __init smdk_pcm_audio_init(void)
{
	int ret;
	unsigned int reg;
	struct clk *sclk, *fout_epll, *mout_epll;

	/* Set Codec clock source
	 * --
	 * To provide 12MHz into WM8580 XTI, Set XCLKOUT to get
	 * clock source from XUSBXTI(24MHz) and make it desired
	 * freq., 12MHz.
	 */
	reg = 0x0;
	/* Clear XCLK_OUT Reg. */
	...
	/* Set XCLK_OUT DIV to make freq. 12MHz */
	reg = __raw_readl(S5P_CLK_OUT);
	...
	/* We use AUDIO_CLK for rate generation, so set EPLLout
	 * as the parent of AUDIO_CLK for PCM.
	 * --
	 * Clock source path looks below,
	 * foutEPLL -> moutEPLL -> Audio-bus -> PCM
	 */
	fout_epll = clk_get(NULL, "fout_epll");
	if (IS_ERR(fout_epll)) {
		printk(KERN_ERR "%s: failed to get fout_epll\n", __func__);
	...
	sclk = clk_get(smdk_dai[0].cpu_dai->dev, "audio-bus");
	if (IS_ERR(sclk)) {
		printk(KERN_ERR "%s: failed to get audio_bus\n", __func__);
		ret = -ENOENT;
		goto err;
	}
