...
static int s3c_pwm_probe(struct platform_device *pdev)
{
	...
	if (id == 0) {
		if(gpio_is_valid(S5PV2XX_GPD0(0))) {
			ret = gpio_request(S5PV2XX_GPD0(0), "GPD");

			if (ret) {
				printk(KERN_ERR "failed to request GPD for PWM-OUT 0\n");
			}
			s3c_gpio_cfgpin(S5PV2XX_GPD0(0), S5PV2XX_GPD_0_0_TOUT_0);			 
		}
	} else if(id == 1) {
		if(gpio_is_valid(S5PV2XX_GPD0(1))) {
			ret = gpio_request(S5PV2XX_GPD0(1), "GPD");

			if (ret) {
				printk(KERN_ERR "failed to request GPD for PWM-OUT 1\n");
			}
			s3c_gpio_cfgpin(S5PV2XX_GPD0(1), S5PV2XX_GPD_0_1_TOUT_1);			 
		}
	
	} else if(id == 2) {
		if(gpio_is_valid(S5PV2XX_GPD0(2))) {
			ret = gpio_request(S5PV2XX_GPD0(2), "GPD");

			if (ret) {
				printk(KERN_ERR "failed to request GPD for PWM-OUT 2\n");
			}
			s3c_gpio_cfgpin(S5PV2XX_GPD0(2), S5PV2XX_GPD_0_2_TOUT_2);			 
		}
	
	} else if(id == 3) {
		if(gpio_is_valid(S5PV2XX_GPD0(3))) {
			ret = gpio_request(S5PV2XX_GPD0(3), "GPD");

			if (ret) {
				printk(KERN_ERR "failed to request GPD for PWM-OUT 3\n");
			}
			s3c_gpio_cfgpin(S5PV2XX_GPD0(3), S5PV2XX_GPD_0_3_TOUT_3);			 
		}
