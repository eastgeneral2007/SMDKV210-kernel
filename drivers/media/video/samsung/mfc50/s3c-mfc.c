...
static int s3c_mfc_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	/*
	 * In the probe process, we calculate exact buffer size align considered.
	 * In case using 1 port, the luma buffer size is 0.
	 */
...
static int s3c_mfc_probe(struct platform_device *pdev)
{
	...
	/* Port 1 */
	s3c_mfc_phys_dpb_luma_buf = Align(pdata->buf_phy_base[1], (128 * BUF_L_UNIT));
