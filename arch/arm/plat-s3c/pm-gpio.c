...
#if defined(CONFIG_ARCH_S3C64XX) || defined(CONFIG_ARCH_S5PV2XX)
...
#if defined(CONFIG_CPU_S5PV210_EVT0)
	tmp = __raw_readl(base + OFFS_DAT);
#endif
	__raw_writel(chip->pm_save[1], base + OFFS_CON);
#if defined(CONFIG_CPU_S5PV210_EVT0)
	tmp = __raw_readl(base + OFFS_CON);
#endif
	if (chip->chip.ngpio > 8) {
		__raw_writel(chip->pm_save[0], base - 4);
#if defined(CONFIG_CPU_S5PV210_EVT0)
		tmp = __raw_readl(base - 4);
#endif
	}
