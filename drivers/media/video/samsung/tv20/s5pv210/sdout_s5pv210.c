...
s5p_tv_sd_err __s5p_sdout_init_vbi(bool wss_cvbs,
	...
	bool wss_y_sideo, s5p_sd_closed_caption_type caption_y_sideo,
	bool cgmsa_rgb, bool wss_rgb,
	s5p_sd_closed_caption_type caption_rgb,	bool cgmsa_y_ppr,
	bool wss_y_ppr,	s5p_sd_closed_caption_type caption_y_ppr)
{
	u32 temp_reg = 0;
					...
					u32 gain)
{
	...
	case SDOUT_CHANNEL_0:
		...
		SDPRINTK("0x%08x\n\r",
			readl(sdout_base + S5P_SDO_SCALE_CH0));
		break;
	...
	case SDOUT_CHANNEL_1:
		...
		SDPRINTK(" 0x%08x\n\r",
			readl(sdout_base + S5P_SDO_SCALE_CH1));
		break;
	...
	case SDOUT_CHANNEL_2:
		...
		SDPRINTK(" 0x%08x\n\r",
			readl(sdout_base + S5P_SDO_SCALE_CH2));
		break;
...
void __s5p_sdout_init_schlock(bool color_sucarrier_pha_adj)
{
	...
	if (color_sucarrier_pha_adj)
		writel(SDO_COLOR_SC_PHASE_ADJ,
			sdout_base + S5P_SDO_SCHLOCK);
	else
		writel(SDO_COLOR_SC_PHASE_NOADJ,
			sdout_base + S5P_SDO_SCHLOCK);
...
void __s5p_sdout_init_oversampling_filter_coeff(u32 size,
	u32 *coeff, u32 *coeff1, u32 *coeff2)
{
	u32 *temp_reg = 0;
	...
	s5p_sd_625_aspect_ratio display_ratio)
{
	...
	SDPRINTK("%d,%d,%d,%d,%d,%d,%d,%d,%d)\n\r", surround_sound, copyright,
		copy_protection, text_subtitles, open_subtitles, camera_film,
		color_encoding, helper_signal, display_ratio);
...
static s5p_tv_sd_err __s5p_sdout_init_antialias_filter_coeff_default(
	s5p_sd_level composite_level, s5p_sd_vsync_ratio composite_ratio,
...
static s5p_tv_sd_err __s5p_sdout_init_oversampling_filter_coeff_default(
	s5p_tv_o_mode out_mode)
{
	SDPRINTK("%d\n\r", out_mode);
					  ...
					  s5p_sd_order order)
{
	...
	case TVOUT_NTSC_M:
		...
		__s5p_sdout_init_antialias_filter_coeff_default(
			S5P_TV_SD_LEVEL_75IRE, SDOUT_VTOS_RATIO_10_4,
			out_mode);
		break;
	...
	case TVOUT_PAL_BDGHI:
		...
		__s5p_sdout_init_antialias_filter_coeff_default(
			S5P_TV_SD_LEVEL_0IRE, SDOUT_VTOS_RATIO_7_3, out_mode);
		break;
	...
	case TVOUT_PAL_60:
		...
		__s5p_sdout_init_video_scale_cfg(S5P_TV_SD_LEVEL_0IRE,
				...
				SDOUT_VTOS_RATIO_7_3);
		__s5p_sdout_init_antialias_filter_coeff_default(
			S5P_TV_SD_LEVEL_0IRE, SDOUT_VTOS_RATIO_7_3,
			out_mode);
		break;
...
void __s5p_sdout_set_interrupt_enable(bool vsync_intr_en)
{
	...
	if (vsync_intr_en)
		writel(readl(sdout_base + S5P_SDO_IRQMASK) &
			~SDO_VSYNC_IRQ_DISABLE,	sdout_base + S5P_SDO_IRQMASK);
	else
		writel(readl(sdout_base + S5P_SDO_IRQMASK) |
			SDO_VSYNC_IRQ_DISABLE, sdout_base + S5P_SDO_IRQMASK);
