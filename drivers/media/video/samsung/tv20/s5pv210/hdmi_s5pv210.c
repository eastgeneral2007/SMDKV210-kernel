...
static s32 hdmi_set_clr_depth(s5p_hdmi_color_depth cd)
{
	...
	default:
		HDMIPRINTK("HDMI core does not support requested\
			Deep Color mode\n");
		return -EINVAL;
	}
					...
					u32 frame_size_code)
{
	...
	writel((readl(hdmi_base + S5P_HDMI_I2S_CH_ST_CON) &
		~(1<<0)) | (1<<0), hdmi_base + S5P_HDMI_I2S_CH_ST_CON);

	writel((readl(hdmi_base + S5P_HDMI_I2S_MUX_CON) &
		~(1<<4 | 3<<2 | 1<<1 | 1<<0)) | (1<<4 | 1<<2 | 1<<1 | 1<<0),
		hdmi_base + S5P_HDMI_I2S_MUX_CON);

	writel((readl(hdmi_base + S5P_HDMI_I2S_MUX_CH) &
		~(0xff<<0)) | (0x3f<<0), hdmi_base + S5P_HDMI_I2S_MUX_CH);

	writel((readl(hdmi_base + S5P_HDMI_I2S_MUX_CUV) &
		~(0x3<<0)) | (0x3<<0), hdmi_base + S5P_HDMI_I2S_MUX_CUV);
...
s5p_tv_hdmi_err __s5p_hdmi_video_init_avi_infoframe(
	s5p_hdmi_transmit trans_type, u8 check_sum, u8 *avi_data)
{
...
s5p_tv_hdmi_err __s5p_hdmi_video_init_mpg_infoframe(
	s5p_hdmi_transmit trans_type, u8 check_sum, u8 *mpg_data)
{
...
irqreturn_t (*hdmi_isr_ftn[HDMI_IRQ_TOTAL_NUM])(int irq);

int s5p_hdmi_register_isr(irqreturn_t (*isr)(int), u8 irq_num)
{
...
irqreturn_t __s5p_hdmi_irq(int irq, void *dev_id)
{
		...
		while (irq_num < HDMI_IRQ_TOTAL_NUM) {
			if (irq_state & (1<<irq_num)) {
				if (hdmi_isr_ftn[irq_num] != NULL)
					...
					HDMIPRINTK("No registered ISR for \
						IRQ[%d]\n", irq_num);
			}
			++irq_num;
		}
	 ...
	 */
	/* HdmiOutp8(S5P_HDMI_INTC_CON,
		1 << HDMI_IRQ_HDCP | 1 << HDMI_IRQ_GLOBAL); */
