...
#define CMM_ALLOC_MAGIC	0x3000	/* must be multiple of PAGE_SIZE */
...
static int cached_size = CONFIG_VIDEO_SAMSUNG_MEMSIZE_CMM_CACHED * SZ_1K;
static int noncached_size = CONFIG_VIDEO_SAMSUNG_MEMSIZE_CMM_NON_CACHED * SZ_1K;

static s3c_cmm_codec_mem_alloc_arg_t *prev_mem_alloc;
		...
		int cmd, unsigned long arg)
{
	...
	case S3C_CMM_IOCTL_CODEC_MEM_ALLOC:
		...
		prev_mem_alloc = &codec_mem_alloc_arg;
		codec_mem_alloc_arg.user_addr = do_mmap(file, 0, codec_mem_alloc_arg.buffSize,
					PROT_READ | PROT_WRITE, MAP_SHARED, CMM_ALLOC_MAGIC);
	...
	case S3C_CMM_IOCTL_CODEC_MEM_FREE:
		...
		do_munmap(mm, codec_mem_free_arg.mapped_addr, codec_mem_free_arg.size);
	...
	case S3C_CMM_IOCTL_CODEC_GET_CACHED_MEMSIZE:
		ret = copy_to_user((void *)arg, (void *)&cached_size,
					sizeof(cached_size));
		break;

	case S3C_CMM_IOCTL_CODEC_GET_NON_CACHED_MEMSIZE:
		ret = copy_to_user((void *)arg, (void *)&noncached_size,
					sizeof(noncached_size));
		break;
...
int s3c_cmm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if ((offset == 0) && (cached_size != 0)) {
		pageFrameNo = __phys_to_pfn(s3c_cmm_buffer_start);
		maxSize = cached_size + PAGE_SIZE -
				(cached_size % PAGE_SIZE);
		vma->vm_flags |= VM_RESERVED | VM_IO;
		size = cached_size;
	} else if (offset == cached_size) {
		pageFrameNo = __phys_to_pfn(s3c_cmm_buffer_start +
						cached_size);
		maxSize = noncached_size + PAGE_SIZE -
				(noncached_size % PAGE_SIZE);
		vma->vm_flags |= VM_RESERVED | VM_IO;
		vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);
		size = noncached_size;
	} else if (offset == CMM_ALLOC_MAGIC) {
		/* get physicall address */
		for (node = s3c_cmm_allocmem_head;
			node != s3c_cmm_allocmem_tail; node = node->next) {
			if (node->u_addr == (unsigned char *)prev_mem_alloc->out_addr)
				break;
		}

		if (node == s3c_cmm_allocmem_tail) {
			CMM_MSG(KERN_DEBUG "\n%s: invalid virtual address(0x%x)\r\n",
					__FUNCTION__, prev_mem_alloc->out_addr);
		}

		if (node->cacheFlag)
			phy_addr = node->cached_p_addr;
		else
			phy_addr = node->uncached_p_addr;

		pageFrameNo = __phys_to_pfn(phy_addr);
		size = prev_mem_alloc->buffSize;
		vma->vm_flags |= VM_RESERVED | VM_IO;
...
static int __init s3c_cmm_init(void)
{
	...
	s3c_cmm_buffer_start = s3c_get_media_memory_bank(S3C_MDEV_CMM, 1);
	...
	node->cacheFlag = 1;
	node->size = cached_size;
	insertnode_freelist(node, -1);

	node = (s3c_cmm_free_mem_t *)kmalloc(sizeof(s3c_cmm_free_mem_t), GFP_KERNEL);
	memset(node, 0x00, sizeof(s3c_cmm_free_mem_t));
	node->startAddr = s3c_cmm_buffer_start + cached_size;
	node->cacheFlag = 0;
	node->size = noncached_size;
	insertnode_freelist(node, -1);
