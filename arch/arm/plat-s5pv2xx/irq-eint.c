/* arch/arm/plat-s5pv2xx/irq-eint.c
 *
...
static inline void s3c_irq_eint_mask(unsigned int irq)
{
	...
	mask = __raw_readl(S5PV2XX_EINTMASK(eint_mask_reg(irq)));
	mask |= eint_irq_to_bit(irq);
	__raw_writel(mask, S5PV2XX_EINTMASK(eint_mask_reg(irq)));
...
static inline void s3c_irq_eint_ack(unsigned int irq)
{
	...
	__raw_writel(eint_irq_to_bit(irq),
			S5PV2XX_EINTPEND(eint_pend_reg(irq)));
#if defined(CONFIG_CPU_S5PV210_EVT0)
	tmp = __raw_readl(S5PV2XX_EINTMASK(eint_mask_reg(irq)));
#endif
}
...
static int s3c_irq_eint_set_type(unsigned int irq, unsigned int type)
{
	...
	ctrl = __raw_readl(S5PV2XX_EINTCON(eint_conf_reg(irq)));
	ctrl &= ~mask;
	ctrl |= newvalue << shift;
	__raw_writel(ctrl, S5PV2XX_EINTCON(eint_conf_reg(irq)));
#if defined(CONFIG_CPU_S5PV210_EVT0)
	ctrl = __raw_readl(S5PV2XX_EINTCON(eint_conf_reg(irq)));
#endif

	ctrl = __raw_readl(S5PV2XX_EINTFLTCON(offs / 4));
	ctrl &= ~(0xff << ((offs & 3) * 8));
	ctrl |= (0x80 << ((offs & 3) * 8));
	__raw_writel(ctrl, S5PV2XX_EINTFLTCON(offs / 4));
#if defined(CONFIG_CPU_S5PV210_EVT0)
	ctrl = __raw_readl(S5PV2XX_EINTFLTCON(offs / 4));
#endif

	if ((0 <= offs) && (offs < 8))
		s3c_gpio_cfgpin(S5PV2XX_GPH0(offs&0x7), 0xf<<((offs&0x7)*4));
	else if ((8 <= offs) && (offs < 16))
		s3c_gpio_cfgpin(S5PV2XX_GPH1(offs&0x7), 0xf<<((offs&0x7)*4));
	else if ((16 <= offs) && (offs < 24))
		s3c_gpio_cfgpin(S5PV2XX_GPH2(offs&0x7), 0xf<<((offs&0x7)*4));
	else if ((24 <= offs) && (offs < 32))
		s3c_gpio_cfgpin(S5PV2XX_GPH3(offs&0x7), 0xf<<((offs&0x7)*4));
	else
...
{
	u32 status = __raw_readl(S5PV2XX_EINTPEND((start >> 3)));
	u32 mask = __raw_readl(S5PV2XX_EINTMASK((start >> 3)));
	unsigned int irq;
...
int __init s5pv2xx_init_irq_eint(void)
{
	int irq;
