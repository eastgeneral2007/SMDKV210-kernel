	...
	u32 priority)
{
	...
	case VM_VIDEO_LAYER:
		layer_cfg =
			S5P_MXR_VP_LAYER_PRIORITY_CLEAR(
...
u32 grp_scaling_factor(u32 src, u32 dst, u32 h_v)
{
	...
	if (dst % src) {
		...
		VMPRINTK(" can't %s scaling src(%d) into dst(%d)\n",
			h_v ? "horizontal" : "vertical", src_w, dst_w);
...
s5p_tv_vmx_err __s5p_vm_init_layer(s5p_tv_disp_mode mode,
	s5p_tv_vmx_layer layer,	bool show, bool win_blending,
	u32 alpha, u32 priority, s5p_tv_vmx_color_fmt color,
	bool blank_change, bool pixel_blending, bool premul, u32 blank_color,
	u32 base_addr, u32 span, u32 width, u32 height, u32 src_offs_x,
	u32 src_offs_y,	u32 dst_offs_x,	u32 dst_offs_y,	u32 dst_width,
	u32 dst_height)
{
	u32 temp_reg = 0;
	u32 h_factor = 0, v_factor = 0;

	VMPRINTK("%d, %d, %d, %d, %d, %d, %d, %d, %d, 0x%x, 0x%x, %d, \
		%d, %d, %d, %d, %d, %d)\n\r",
...
s5p_tv_vmx_err __s5p_vm_init_bg_color(s5p_tv_vmx_bg_color_num color_num,
					    ...
					    u32 color_cr)
{
	return __s5p_vm_set_bg_color(color_num, color_y, color_cb,
		color_cr);
}
