...
static int wm8753_register(struct wm8753_priv *wm8753)
{
	...
	/* disable output volume for reduce popup noise */
...
/*
 * If there are no alsa state in rootfs, following codes make
 * sound enable without sound pre-set state.
 *
 * Belows are command line set-up for Vogue board audio test.
 * - audio playback enable for vogue board
 *  amixer sset 'Left Mixer Left' on
 *  amixer sset 'Right Mixer Right' on
 * - audio line-in enable for vogue board
 *  amixer sset 'DAI Mode' 'DAI 2'
 *  amixer sset 'Capture' cap
 *  amixer sset 'ALC Mixer Line' cap
 *  amixer sset 'ALC Capture Function' Right
 */
#if 0
	/* set the power-on by default */
	reg = wm8753_read_reg_cache(codec, WM8753_PWR1);
	wm8753_write(codec, WM8753_PWR1, reg | 0x00fd);
	reg = wm8753_read_reg_cache(codec, WM8753_PWR2);
	wm8753_write(codec, WM8753_PWR2, reg | 0x01ff);
	reg = wm8753_read_reg_cache(codec, WM8753_PWR3);
	wm8753_write(codec, WM8753_PWR3, reg | 0x01fe);
	reg = wm8753_read_reg_cache(codec, WM8753_PWR4);
	wm8753_write(codec, WM8753_PWR4, reg | 0x000f);

	reg = wm8753_read_reg_cache(codec, WM8753_LOUTM1);
	wm8753_write(codec, WM8753_LOUTM1, reg | 0x0188);
	reg = wm8753_read_reg_cache(codec, WM8753_ROUTM1);
	wm8753_write(codec, WM8753_ROUTM1, reg | 0x0188);

	/* DAC mute off */
	reg = wm8753_read_reg_cache(codec, WM8753_DAC);
	wm8753_write(codec, WM8753_DAC, reg & 0xfff7);
