/* linux/arch/arm/plat-s5pv2xx/clock.c
 *
...
static int powerdomain_set(struct powerdomain *pd, int enable)
{
	...
	if (enable) {
		...
		do {
			reg_dat = __raw_readl(stable_reg);
		} while (!(reg_dat & ctrlbit));
...
static void s5pv2xx_register_clks_on_pd(struct clk *clks)
{
...
static inline int s5pv2xx_clk_gate(void __iomem *reg,
				struct clk *clk,
				int enable)
{
...
int s5pv2xx_clk_sclk1_ctrl(struct clk *clk, int enable)
{
	return s5pv2xx_clk_gate(S5P_CLKGATE_SCLK1, clk, enable);
}

int s5pv2xx_clk_ip0_ctrl(struct clk *clk, int enable)
{
	if ((clk->ctrlbit)&(S5P_CLKGATE_IP0_RESERVED|S5P_CLKGATE_IP0_ALWAYS_ON))
		return 0;

	return s5pv2xx_clk_gate(S5P_CLKGATE_IP0, clk, enable);
}

int s5pv2xx_clk_ip1_ctrl(struct clk *clk, int enable)
{
	if ((clk->ctrlbit)&(S5P_CLKGATE_IP1_RESERVED|S5P_CLKGATE_IP1_ALWAYS_ON))
		return 0;

	return s5pv2xx_clk_gate(S5P_CLKGATE_IP1, clk, enable);
}

int s5pv2xx_clk_ip2_ctrl(struct clk *clk, int enable)
{
	if ((clk->ctrlbit)&(S5P_CLKGATE_IP2_RESERVED|S5P_CLKGATE_IP2_ALWAYS_ON))
		return 0;

	return s5pv2xx_clk_gate(S5P_CLKGATE_IP2, clk, enable);
}

int s5pv2xx_clk_ip3_ctrl(struct clk *clk, int enable)
{
	if ((clk->ctrlbit)&(S5P_CLKGATE_IP3_RESERVED|S5P_CLKGATE_IP3_ALWAYS_ON))
		return 0;

	return s5pv2xx_clk_gate(S5P_CLKGATE_IP3, clk, enable);
}

int s5pv2xx_clk_ip4_ctrl(struct clk *clk, int enable)
{
	if ((clk->ctrlbit)&(S5P_CLKGATE_IP4_RESERVED|S5P_CLKGATE_IP4_ALWAYS_ON))
		return 0;

	return s5pv2xx_clk_gate(S5P_CLKGATE_IP4, clk, enable);
}

int s5pv2xx_clk_block_ctrl(struct clk *clk, int enable)
{
	return s5pv2xx_clk_gate(S5P_CLKGATE_BLOCK, clk, enable);
}

int s5pv2xx_clk_bus0_ctrl(struct clk *clk, int enable)
{
	return s5pv2xx_clk_gate(S5P_CLKGATE_BUS0, clk, enable);
}

int s5pv2xx_clk_bus1_ctrl(struct clk *clk, int enable)
{
	return s5pv2xx_clk_gate(S5P_CLKGATE_BUS1, clk, enable);
}

int s5pv2xx_audss_clkctrl(struct clk *clk, int enable)
{
	return s5pv2xx_clk_gate(S5P_CLKGATE_AUDSS, clk, enable);
}
 ...
 * In kernel booting sequence, basically disable all IP's clock and
 * MM power domain.
 * If D/D uses specific clock, use clock API.
 */
void s5pv2xx_init_clocks_power_disabled(void)
{
	...
	do {
		...
		s5pv2xx_clk_ip0_ctrl(&clk_dummy, 0);
		s5pv2xx_clk_ip1_ctrl(&clk_dummy, 0);
		s5pv2xx_clk_ip2_ctrl(&clk_dummy, 0);
		s5pv2xx_clk_ip3_ctrl(&clk_dummy, 0);
		s5pv2xx_clk_ip4_ctrl(&clk_dummy, 0);
...
void __init s5pv2xx_register_clocks(void)
{
	struct clk *clkp;
	int ret;
	int ptr;
	...
	for (ptr = 0; ptr < ARRAY_SIZE(init_clocks); ptr++, clkp++) {
		...
		s5pv2xx_register_clks_on_pd(clkp);
	}

	s5pv2xx_init_clocks_power_disabled();
}
