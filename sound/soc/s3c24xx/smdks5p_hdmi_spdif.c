 ...
 *
 * - ALSA dummy codec driver for SPDIF transmitter
 */
...
/*
 * SPDIF supports various sampling rates of 32KHz~96KHz.
...
static struct snd_soc_dai_ops s5p_hdmi_spdif_dai_ops = {
	.hw_params  = NULL,
	.set_fmt    = NULL,
	.set_clkdiv = NULL,
	.set_pll    = NULL,
				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	s3c_gpio_cfgpin(S5PC1XX_GPG3(5), S3C_GPIO_SFN(5));     /* spdif_0_out */
	s3c_gpio_setpull(S5PC1XX_GPG3(5), S3C_GPIO_PULL_UP);
	s3c_gpio_cfgpin(S5PC1XX_GPG3(6), S3C_GPIO_SFN(5));    /* spdif_extcal */
	s3c_gpio_setpull(S5PC1XX_GPG3(6), S3C_GPIO_PULL_UP);
#else
#ifdef CONFIG_ARCH_S5PV2XX
	s3c_gpio_cfgpin(S5PV2XX_GPC1(0), S3C_GPIO_SFN(3));     /* spdif_0_out */
	s3c_gpio_setpull(S5PV2XX_GPC1(0), S3C_GPIO_PULL_UP);
	s3c_gpio_cfgpin(S5PV2XX_GPC1(1), S3C_GPIO_SFN(3));    /* spdif_extcal */
	s3c_gpio_setpull(S5PV2XX_GPC1(1), S3C_GPIO_PULL_UP);
#endif
#endif
	return 0;
}
...
static int __init smdks5p_audio_init(void)
{
	...
	snd_soc_register_dais(s5p_hdmi_spdif_dai,
			      ARRAY_SIZE(s5p_hdmi_spdif_dai));
...
{
	snd_soc_unregister_dais(s5p_hdmi_spdif_dai,
				ARRAY_SIZE(s5p_hdmi_spdif_dai));
