...
/* MFC fw 10/30, EVT0 */
	...
	/* Mask interrupt */
	WRITEL(0, S3C_FIMV_ENC_INT_MASK);
						      ...
						      args)
{
	...
	if (mfc_ctx->dynamic_framerate != 0) {
		...
		if (mfc_ctx->MfcCodecType == MPEG4_ENC) {
			time_increment_res =
				mfc_ctx->dynamic_framerate * MPEG4_TIME_RES;
			WRITEL_SHARED_MEM(
				(1 << 31) |
