...
void __s5p_vp_set_field_id(s5p_vp_field mode)
{
	...
	writel((mode == VPROC_TOP_FIELD) ? vp_base + S5P_VP_FIELD_ID_TOP :
		vp_base + S5P_VP_FIELD_ID_BOTTOM, vp_base + S5P_VP_FIELD_ID);
				...
				u32 src_off_y)
{
	...
	writel(VP_SRC_V_POSITION(src_off_y),
...
void __s5p_vp_set_poly_filter_coef_default(u32 h_ratio, u32 v_ratio)
{
	...
	/*
	* For the real interlace mode, the vertical ratio should be used
	* after divided by 2. Because in the interlace mode, all the VP
	* output is used for SDOUT display and it should be the same as
	* one field of the progressive mode. Therefore the same filter
	* coefficients should be used for the same the final output video.
...
{
	u32 temp_reg;
	VPPRINTK("%d,%d,%d,%d\n\r", line_skip, mem_mode, chroma_exp,
		toggle_id);
	...
	u32 dst_height, bool ipc_2d)
{
	...
	__s5p_vp_set_src_dest_size(src_width, src_height,
...
s5p_tv_vp_err __s5p_vp_init_layer_def_poly_filter_coef(u32 top_y_addr,
	...
	u32 src_width, u32 src_height, u32 dst_off_x, u32 dst_off_y,
	u32 dst_width, u32 dst_height, bool ipc_2d)
{
	...
	error = __s5p_vp_init_layer(top_y_addr, top_c_addr,
			bottom_y_addr, bottom_c_addr,
			src_img_endian, img_width, img_height,
			src_off_x, src_x_fract_step, src_off_y,
			src_width, src_height, dst_off_x, dst_off_y,
			dst_width, dst_height, ipc_2d);
	return error;
}
	...
	s5p_vp_sharpness_control sharpness)
{
	...
	writel(VP_TH_HNOISE(th_h_noise) |
		VP_SHARPNESS(sharpness),
		vp_base + S5P_PP_SHARPNESS);

	VPPRINTK("0x%08x\n\r", readl(vp_base + S5P_PP_SHARPNESS));
}

s5p_tv_vp_err __s5p_vp_init_brightness_contrast_control(
	s5p_vp_line_eq eq_num, u32 intc, u32 slope)
{
	return __s5p_vp_set_brightness_contrast_control(eq_num,
		intc, slope);
}
